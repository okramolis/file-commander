...
const ARRAY_QUOTED_DELIMITER    = '", "'
  ...
  ,   APP_ERR                   = {
      ...
      , FS_READ_MIME            : 'An error occurred while trying to read a file mime type at "%s".'
  ...
  ,   APP_WARN                  = {
      ...
      , UPLOAD_REQ_TOO_BIG      : 'Submitted files for upload are too big. Size limit is %sMB per request.'
      }
  ,   APP_DESC                  = {
  ...
  ,   UPLOAD_ERR_REQ_TOO_BIG    = 'ETOOBIG'
  ...
  ,   MAIN_APP_HOME_HASH        = '#/folders'
      // TODO let the mimes be passed by application
  ,   MIME_SUPPORT              = {
  ...
  ,   MIME_EXT_DEFAULT          = null
      // TODO let the limit be passed by application
  ,   UPLOAD_MAX_BYTES_PER_REQ  = Math.pow(2,20)
...
var express = require('express')
  ...
  , mimeExt = require('mime')
  , async = require('async')
  , uploader = require('multiparty')
...
// set default mime type for extension based mime module
mimeExt.default_type = MIME_EXT_DEFAULT;

// -------------------------------------------------------------------
// CUSTOM MIDDLEWARE DEFINITIONS
// -------------------------------------------------------------------

// prevents fs error to occur when not a file by reading stats of the current path
function fsFileSafeMiddleware(req, res, next) {
  if (req.fcmder.fs.path.stats.isFile) {
    // get mime type of the requested file
    fsMimeMiddleware(req, res, next);
    return;
  }
  next();
} // END of fsFileSafeMiddleware

// stores mime type of requested file in req object
function fsMimeMiddleware(req, res, next) {
  var fspath = req.fcmder.fs.path;
  utilMimeGet(fspath.mount + fspath.name, function(err, type) {
    if (err) {
      // ... ERROR occured => pass the request to the error handler or next route
      utilHandleFsMiddlewareError(err, next, APP_ERR.FS_READ_MIME, fspath.name);
      return;
    }
    // ... mime SUCCESSFULLY read => pass the request to the next
    fspath.mime = type;
    next();
  });
} // END of fsMimeMiddleware
...
function fsStatFilesSafeMiddleware(req, res, next) {
  ...
  next();
} // END of fsStatFilesSafeMiddleware
...
function fsStatFilesMiddleware(req, res, next) {
  ...
  async.each(ofs.files.names, function(file, eachNext) {
    var filepath = path.join(ofs.path.name, file)
      , mount = ofs.path.mount
    ;
    async.waterfall([
      // metadata
      fsStatFilesMiddlewareItem.stats.bind(null, mount, filepath),
      // mime type
      fsStatFilesMiddlewareItem.mime.bind(null, mount, filepath)
    ], function(err, stats, type) {
      if (err) {
        eachNext(err);
        return;
      }
      // store results
      ofs.files.stats[file] = stats;
      ofs.files.mimes[file] = type;
      // go to next file
      eachNext();
    });
  }, next);
...
// inner middleware-like utils of fsStatFilesMiddleware middleware
// - called for each file
var fsStatFilesMiddlewareItem = {
  stats: function (mount, filepath, callback) {
    // read metadata about file system path
    fs.lstat(mount + filepath, function(err, stats) {
      if (err) {
        // ... ERROR occured => pass the request to the error handler or next route
        utilHandleFsMiddlewareError(err, callback, APP_ERR.FS_STAT, filepath);
        return;
      }
      // ... file SUCCESSFULLY stated => continue with the next one
      callback(null, utilTransformFsStats(stats));
    });
  }, // END of stats

  mime: function(mount, filepath, stats, callback) {
    if (!stats.isFile) {
      // not a file - go to next immediately
      callback(null, stats);
      return;
    }
    // get mime type of the file
    utilMimeGet(mount + filepath, function(err, type) {
      if (err) {
        // ... ERROR occured => pass the request to the error handler or next route
        utilHandleFsMiddlewareError(err, callback, APP_ERR.FS_READ_MIME, filepath);
        return;
      }
      // ... mime SUCCESSFULLY read => pass the request to the next
      callback(null, stats, type);
    });
  } // END of mime
}; // END of fsStatFilesMiddlewareItem

// simulates express.static middleware
function getFsStaticMiddleware(settings) {
  return function(req, res, next) {
    var fspath = req.fcmder.fs.path;
    if (fspath.stats.isFile || fspath.stats.isLink) {
      // send file or link
      res.sendFile(fspath.mount + fspath.name, settings);
      return;
...
function fsMovementsMiddleware(req, res, next) {
  ...
  if (!path.basename(source) || !utilFsIsAbsolutePath(source)) {
    ...
    utilSetReqState(
      req,
      utilWarnCode2Msg(APP_WARN.INVALID_REQUEST_ATTR),
      utilDescCode2Msg(APP_DESC.INVALID_ATTRIBUTE_FORMAT, 'local', 'an absolute path but not a root path'),
      403
    );
    next('route');
    return;
...
function fsUploadMiddleware(req, res, next) {
  var fsmount = req.fcmder.fs.path.mount
    ...
    , uploadDir = fsmount + fsname
    , form = new uploader.Form()
    , files = []
  ;

  // write all files or nothing
  var receiveAllParts = null;
  form.on('part', function(part) {
    var field = part.name
      , fname = part.filename
    ;
    if (fname === null) {
      // field name - ignore it
      part.resume();
    } else {
      // file data - handle it
      if (receiveAllParts === null) {
        // check size limit for whole request
        receiveAllParts = part.byteCount <= UPLOAD_MAX_BYTES_PER_REQ;
      }
      if (receiveAllParts) {
        // OK - there is enough space for the rest of the request
        // compose temporary path where the uploaded data are going to be written
        var tmpPath = path.join(uploadDir, utilGenHexId(32));
        // create destination on disk for data
        var dest = fs.createWriteStream(tmpPath);
        // compose arguments for post upload processing
        var ofile = {
          dest: {
            wstream: dest,
            closed: false
        ...
        dest.on('close', function() {
          ofile.dest.closed = true;
        });
        // start writing data to disk to the temporary path
        part.pipe(dest);
        // store file object
        files.push([field, ofile]);
      } else {
        // OVER LIMIT - there are more bytes to receive than bytes allowed to be recieved
        part.resume();
      }
    }
    part.on('error', function(err) {
      // TODO handle part 'error' event properly
      console.log('file-commander: upload file form part error:');
      console.log(err);
    });
  });

/*
  // write at least LAST files that satisfy the size limit
  var bytesWritten = 0
    , lastCount = null
  ;
  form.on('part', function(part) {
    ...
    if (fname === null) {
      ...
      // file data - handle it
      if (lastCount !== null) {
        bytesWritten += lastCount - part.byteCount;
      }
      // check size limit
      if (part.byteCount <= (UPLOAD_MAX_BYTES_PER_REQ - bytesWritten)) {
        // OK - there is enough space for the rest of the request
        // compose temporary path where the uploaded data are going to be written
        var tmpPath = path.join(uploadDir, utilGenHexId(32));
        // start writing data to disk to the temporary path
        ...
        // remember current bytes count to support bytes written counter
        lastCount = part.byteCount;
      } else {
        // OVER LIMIT - there are more bytes to receive than bytes allowed to be recieved
        part.resume();
        lastCount = null;
...
*/
  ...
  form.on('error', function(err) {
    if (utilMatchError(err, UPLOAD_ERR_REQ_TOO_BIG)) {
      // too big data - whole request
      utilSetReqState(
        req,
        utilWarnCode2Msg(APP_WARN.UPLOAD_REQ_TOO_BIG, UPLOAD_MAX_BYTES_PER_REQ / MEGA_BYTE),
        null,
        413
      );
    } else {
      // unexpected error occured during upload
      utilSetReqState(
        req,
    ...
    // make sure all temporary files are deleted
    utilFsPostUploadCleanup(files);
  });
...
function reqInitMiddleware(req, res, next) {
  ...
  if (req.session && !req.session.hasOwnProperty('fcmder')) {
    req.session.fcmder = {};
  }
  next();
...
function utilFsIsParent(parentPath, childPath) {
  var childDir = path.dirname(path.resolve(childPath))
      // TODO make this platform independent (path.sep instead of slash char)
...
// TODO make this platform independent (maybe use path.resolve)
...
function utilFsPostUpload(files, dest, callback) {
  ...
  async.each(files, function(fileArr, next) {
    ...
    ;
    if (file.dest.closed) {
      utilFsPostUploadPreFileMove(fname, fpath, move, notMoved, next);
      return;
    }
    file.dest.wstream.on(
      'close',
      utilFsPostUploadPreFileMove.bind(null, fname, fpath, move, notMoved, next)
    );
  }, function(err) {
    if (err) {
...
// Performs necessary checks before the uploaded file is moved
// from its temporary location to its final destination.
function utilFsPostUploadPreFileMove(fname, fpath, move, notMoved, next) {
  // check file name format - 400 bad request
  if (!utilFsValidateFilename(fname)) {
    // invalid file name
    // delete temporary file and go to next file
    notMoved.push(fname);
    fs.unlink(fpath, next);
    return;
  }
  // check real mime type and see if allowed
  mime(fpath, function(err, type) {
    if (err) {
    ...
    // check file name uniqueness right before adding the file to the collection
    // - 409 conflict
    if (!_.isUndefined(move[fname])) {
...
} // END of utilFsPostUploadPreFileMove
...
// combines two approaches to getting mime type of file
// - at first it tries to lookup mime type in hash table using file path name
// - if the look up fails, then it tries to read the type from file
function utilMimeGet(fspath, callback) {
  var type = mimeExt.lookup(fspath);
  if (type !== null) {
    // valid mime type looked up in table
    process.nextTick(callback.bind(null, null, type));
    return;
  }
  // valid mime type NOT found in table
  // - try to read mime type from file
  mime(fspath, callback);
} // END of utilMimeGet
...
function createServerBased(settings) {
  ...
  switch (APP_TYPE) {
    ...
    case APP_TYPE_PRIVATE:
      ...
      .get(
        getFsStaticMiddleware(STATIC_SETTINGS),
        fsDirectoryMiddleware,
        fsStatFilesMiddleware
      )
      .post(
...
// static methods
Commander.setRequestPrivateDir = function(req, mount) {
  req.fcmder.fs.path = {
    mount : mount
  };
}
Commander.setRequestUserHome = function(req, mount) {
  console.log('file-commander: please use Commander.setRequestPrivateDir ' +
              'instead of Commander.setRequestUserHome as this method is ' +
              'going to be removed in v0.2');
